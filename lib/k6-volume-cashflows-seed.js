 ...
 *
 * - **cashflows-timeline** — GET Events/default + custom, POST …/timelines/events, GET timelines/financing
 * - **cashflows-income** — GET income-expense/financial, POST financial/income, resolveIncomeLineAfterPost
 * - **cashflows-finances** — POST financial/expense, resolveExpenseLineAfterPost, GET …/financial
 *   — GET/POST …/funds (contributions & withdrawals)
...
/**
 * **cashflows-timeline** module sequence.
 */
export function seedVolumeTimelineModule(ctx) {
  const { base, hdrs, cashflowId, birthIso, timeout, observe } = ctx;
  ...
  const timelineEvents = pickTimelineEventsFromDefaults(parseJsonArray(resEvDef), birthYear);
  let postsOk = 0;
  ...
  return {
    ok: postsOk > 0 || (resTl.status === 200 && timelineEvents.length === 0),
    eventsPosted: postsOk,
  };
}

/**
 * **cashflows-income** module sequence (persistent lines, no delete).
 */
export function seedVolumeIncomeModule(ctx) {
  ...
  check(resIe, { 'fp income: GET income-expense/financial 200': (r) => r.status === 200 });

  const paramList = buildRealisticIncomeLineItemParams({ persona, birthYear, planIndex, clientTag });
  let resolvedCount = 0;
  for (let i = 0; i < paramList.length; i++) {
    const body = buildMinimalIncomeLineItem(paramList[i]);
    const resPost = http.post(`${base}/api/v1/cashflows/${cfEnc}/financial/income`, JSON.stringify(body), {
      headers: hdrs,
      tags: { name: 'cashflows_income_post_income', vu: String(vuHint) },
      timeout,
    });
    observe(resPost, {
      method: 'POST',
      endpoint: '/api/v1/cashflows/{cashflowId}/financial/income',
      tagName: 'fp_finances_post_income',
    });
    ...
    const resolved = resolveIncomeLineAfterPost({
      ...
      stablePrefix: marker,
      vuTag: String(vuHint),
      tagName: 'fp_volume_income_resolve',
      maxAttempts: 8,
    });
    if (resolved && resolved.id) resolvedCount += 1;
  }

  check(null, {
    'fp income: at least one income line persisted': () => resolvedCount >= 1,
  });
  return { ok: resolvedCount >= 1, lines: resolvedCount };
}

/**
 * **cashflows-finances** module sequence.
 */
export function seedVolumeFinancesModule(ctx) {
  ...
  const paramList = buildRealisticExpenseLineItemParams({ persona, birthYear, planIndex, clientTag });
  let resolvedCount = 0;
  for (let i = 0; i < paramList.length; i++) {
    const body = buildMinimalExpenseLineItem(paramList[i]);
    const resPost = http.post(`${base}/api/v1/cashflows/${cfEnc}/financial/expense`, JSON.stringify(body), {
      headers: hdrs,
      tags: { name: 'cashflows_finances_post_expense', vu: String(vuHint) },
      timeout,
    });
    observe(resPost, {
      method: 'POST',
      endpoint: '/api/v1/cashflows/{cashflowId}/financial/expense',
      tagName: 'fp_finances_post_expense',
    });
    if (!httpOk(resPost.status)) continue;
    const resolved = resolveExpenseLineAfterPost({
      ...
      stablePrefix: marker,
      vuTag: String(vuHint),
      tagName: 'fp_volume_expense_resolve',
      maxAttempts: 8,
    });
    if (resolved && resolved.id) resolvedCount += 1;
  }
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    if (finCounts.incomeCount >= 1 && finCounts.expenseCount >= 1) break;
  }

  check(null, {
    'fp finances: full record has income and expense': () =>
      finCounts.incomeCount >= 1 && finCounts.expenseCount >= 1,
  });
  return {
    ok: resolvedCount >= 1 && finCounts.incomeCount >= 1 && finCounts.expenseCount >= 1,
    expenseLines: resolvedCount,
    finCounts,
  };
}
...
export function seedRealisticPlanData(p) {
  ...
  const income = seedVolumeIncomeModule(ctx);
  ...
  return {
    ...
    incomeOk: income.ok,
    financesOk: finances.ok,
    contributionsOk: funds.ok,
    reportOk: reports.ok,
    wealthOk: wealth.ok,
    ok: timeline.ok && income.ok && finances.ok && funds.ok && reports.ok && wealth.ok,
  };
}
