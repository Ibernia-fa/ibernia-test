...
export function purgeK6ClientsAndPlansForAdvisor(
  ...
  const exclude = normalizeExcludeClientIds(opts.excludeClientIds);
  const tn = tagNames || {};
 ...
 */
export function deleteClientsAndPlansByLastNameNeedle(
  base,
  hdrs,
  advisorSub,
  needle,
  ...
  const tn = tagNames || {};
  const exclude = normalizeExcludeClientIds(options && options.excludeClientIds);
  const listed = listAdvisorClients(base, hdrs, advisorSub, httpTimeout, tn);
  if (!listed.ok) return;
  const clientIds = [];
  for (let i = 0; i < listed.clients.length; i++) {
    const row = listed.clients[i];
    if (!clientRowMatchesUniqueTag(row, needle)) continue;
    const id = clientIdFromListRow(row);
    if (!id || exclude.has(id)) continue;
    clientIds.push(id);
  }
