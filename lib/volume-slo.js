...
import { normalizeVolumeConfigOpenPath } from './volume-http-retry.js';
 ...
 * Absolute paths from PowerShell are normalized to `config/...` when possible.
 * @param {string} preferred
 * @param {readonly string[]} fallbacks
 */
function normalizeOpenPath(preferred) {
  return normalizeVolumeConfigOpenPath(preferred);
}

function isAbsoluteFilesystemOpenPath(p) {
...
function openJsonWithFallbacks(preferred, fallbacks) {
  const seen = new Set();
  const normalized = normalizeOpenPath(preferred);
  const paths = [];
  if (normalized) {
    paths.push(`../${normalized}`, `../../${normalized}`, normalized);
  }
  if (preferred && !isAbsoluteFilesystemOpenPath(preferred)) {
    paths.unshift(preferred);
  }
  paths.push(...(fallbacks || []));
