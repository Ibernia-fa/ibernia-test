...
/** Matches Ibernia portal default when commission is disabled. */
export function emptyFundLineCommission() {
  return {
    type: 0,
...
export function buildMinimalFundTransactionLineItem(p) {
  ...
  const defaultContributionType = p.isWithdrawal === true ? 0 : 1;
  const line = {
    description: desc,
    amount: { amount: amountVal, currencySymbol: '€', cycle },
    start: { age: startAge, year: startYear },
    end: { age: endAge, year: endYear },
    contributionType: p.contributionType != null ? p.contributionType : defaultContributionType,
    escalationRate: p.escalationRate ?? { description: '', value: 0 },
    hasCommission: p.hasCommission === true,
    comission: p.comission ?? emptyFundLineCommission(),
  };
