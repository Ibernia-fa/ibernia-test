...
/** SavingPotType.Investment / Other — matches Savings “Add new +” presets (not default Cash). */
...
export function selectPersonaForClient(clientTag, clientIndex, planIndex = 0) {
  ...
  // clientIndex * 7 was ≡ -clientIndex (mod 8), which cancelled +1 hash steps from c1,c2,… suffixes.
...
function lineDescription(label, marker, lineKey) {
  const key = lineKey ? `${marker}|${lineKey}` : marker;
  return `${label} [${key}]`;
}
