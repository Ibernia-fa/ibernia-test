...
export function buildPhaseAAdvisorExecutionSummary(ctx = {}) {
  ...
  const dimensions = buildPhaseAResultDimensions({
    validation,
    totals,
  ...
  /** @type {string[]} */
  const dataFailureReasons = [];
  if (dimensions.manifestValidation === false) {
    dataFailureReasons.push('Manifest validation failed');
  }
  if (dimensions.dataSeeding === false) {
    if (expectedClients != null && totals.clients !== expectedClients) {
      dataFailureReasons.push(`Clients seeded: ${totals.clients}/${expectedClients}`);
    }
    if (expectedPlans != null && totals.plans !== expectedPlans) {
      dataFailureReasons.push(`Plans seeded: ${totals.plans}/${expectedPlans}`);
    }
    if (failedAdvisorJobs > 0) {
      dataFailureReasons.push(`${failedAdvisorJobs} advisor job(s) failed at process level`);
    }
    if (partialAdvisorResults > 0) {
      dataFailureReasons.push(`${partialAdvisorResults} advisor(s) with partial or missing seed data`);
    }
    if (runMeta && runMeta.manifestCollected != null && advisorCount > 0) {
      ...
      if (collected < advisorCount) {
        dataFailureReasons.push(`Manifest shards collected: ${collected}/${advisorCount}`);
      }
    }
  }

  /** @type {string[]} */
  const performanceFailureReasons = [];
  if (advisorsOverLatency > 0) {
    performanceFailureReasons.push(`${advisorsOverLatency} advisors exceeded latency budgets`);
  }
  ...
  if (worstCreateClient != null && advisorsOverLatency > 0) {
    performanceFailureReasons.push(`Worst create-client latency: ${worstCreateClient} ms`);
  }
  if (dimensions.fleetSloGate === false) {
    performanceFailureReasons.push('Fleet SLO gate failed');
  }
  ...
  return {
    ...
    quotaSummary,
    dimensions,
    overallResult: dimensions.overallResult,
    /** @deprecated Use overallResult === 'PASS' */
    overallPass: dimensions.overallResult === 'PASS',
    dataFailureReasons,
 ...
 *   totals?: { clients?: number, plans?: number },
 *   sloGate?: { passed?: boolean }|null,
 *   advisorsOverLatency?: number,
 * }} ctx
 */
export function buildPhaseAResultDimensions(ctx = {}) {
  const validation = ctx.validation || null;
  const totals = ctx.totals || { clients: 0, plans: 0 };
  const sloGate = ctx.sloGate || null;
  const advisorsOverLatency = ctx.advisorsOverLatency != null ? Number(ctx.advisorsOverLatency) : 0;

  const expectedClients =
    validation && validation.expectedClients != null ? Number(validation.expectedClients) : null;
  const expectedPlans =
    validation && validation.expectedPlans != null ? Number(validation.expectedPlans) : null;
  ...
  const manifestValidation = validation ? !!validation.passed : null;
  const fleetSloGate = sloGate ? !!sloGate.passed : null;
  const advisorLatencyCompliance = advisorsOverLatency === 0;
  const performanceCertification = fleetSloGate === true && advisorLatencyCompliance;

  const dataSuccess = dataSeeding === true && manifestValidation === true;

  /** @type {'PASS'|'PASS WITH PERFORMANCE VIOLATIONS'|'FAIL'} */
  let overallResult;
  if (!dataSuccess) {
    overallResult = 'FAIL';
  } else if (performanceCertification) {
    overallResult = 'PASS';
  } else {
    overallResult = 'PASS WITH PERFORMANCE VIOLATIONS';
  }
...
export function buildPhaseASummaryMarkdown(ctx) {
  ...
  const fmtDimension = (v) => (v === null || v === undefined ? 'n/a' : v ? 'PASS' : 'FAIL');
  const dims = exec.dimensions;

  const lines = [
    `# Phase A volume summary — ${runTag}`,
    '',
    `# Overall Result: ${exec.overallResult}`,
    '',
  ];

  if (exec.overallResult === 'PASS') {
    lines.push('All data gates passed.');
    lines.push('Manifest validation passed.');
    lines.push('Fleet SLO gate passed.');
    lines.push('Advisor latency compliance passed.');
    lines.push('');
  } else if (exec.overallResult === 'PASS WITH PERFORMANCE VIOLATIONS') {
    lines.push('Data seeding completed successfully.');
    lines.push('Manifest validation passed.');
    lines.push('');
    lines.push('However:');
    lines.push('- Fleet and/or advisor latency requirements were not met.');
    lines.push('');
  } else {
    lines.push('Data seeding or manifest validation failed.');
    lines.push('');
  }
  ...
  lines.push('|------------|---------|');
  lines.push(`| Data seeding | ${fmtDimension(dims.dataSeeding)} |`);
  lines.push(`| Manifest validation | ${fmtDimension(dims.manifestValidation)} |`);
  lines.push(`| Fleet SLO gate | ${fmtDimension(dims.fleetSloGate)} |`);
  lines.push(`| Advisor latency compliance | ${fmtDimension(dims.advisorLatencyCompliance)} |`);
  lines.push(`| Performance certification | ${fmtDimension(dims.performanceCertification)} |`);
  lines.push('');
  lines.push(`Generated: ${ctx.generatedAt || new Date().toISOString()}`);
  lines.push('');

  if (exec.dataFailureReasons.length) {
    lines.push('## Data failures');
    lines.push('');
    for (const r of exec.dataFailureReasons) {
      lines.push(`- ${r}`);
    }
    lines.push('');
  }

  if (exec.performanceFailureReasons.length) {
    lines.push('## Performance failures');
    lines.push('');
    for (const r of exec.performanceFailureReasons) {
      lines.push(`- ${r}`);
    }
    lines.push('');
  }
