...
import { volumeFleetStaggerEnabledFromEnv } from './k6-load-cleanup-core.js';
import {
...
function volumeFleetStaggerEnabled() {
  const counts = resolvePhaseAVolumeCounts();
  return volumeFleetStaggerEnabledFromEnv(__ENV, counts.clientsPerAdvisor);
}

function postClientOnce({ base, authCtx, createBody, timeout, clientTag }) {
  authCtx.ensureFresh(false);
  let resCreate = http.post(`${base}/api/v1/Clients`, JSON.stringify(createBody), {
    headers: authCtx.hdrs,
    tags: { name: 'fp_clients_post' },
    timeout,
  });
  fpObserve(resCreate, { method: 'POST', endpoint: '/api/v1/Clients', tagName: 'fp_clients_post' });
  let parsed = parseClientCreateResponse(resCreate);
  if ((!parsed.id || !parsed.model) && resCreate.status === 401 && authCtx.refreshAccessToken) {
    console.warn(`[volume-client] 401 on create — refreshing token clientTag=${clientTag}`);
    authCtx.ensureFresh(true);
    resCreate = http.post(`${base}/api/v1/Clients`, JSON.stringify(createBody), {
      headers: authCtx.hdrs,
      tags: { name: 'fp_clients_post' },
      timeout,
    });
    fpObserve(resCreate, { method: 'POST', endpoint: '/api/v1/Clients', tagName: 'fp_clients_post' });
    parsed = parseClientCreateResponse(resCreate);
  }
  if (!parsed.id) {
    const status = resCreate ? resCreate.status : 'n/a';
    let bodyHint = '';
    if (resCreate && resCreate.body) {
      const raw = String(resCreate.body);
      bodyHint = raw.length > 240 ? `${raw.slice(0, 240)}…` : raw;
    }
    console.error(
      `[volume-client] create failed clientTag=${clientTag} status=${status}` +
        (bodyHint ? ` body=${bodyHint}` : ''),
    );
  }
  return { resCreate, clientId: parsed.id, modelRaw: parsed.model };
}

function postCashflowOnce({ base, authCtx, body, timeout, clientTag, planIndex }) {
  authCtx.ensureFresh(false);
  let res = http.post(`${base}/api/v1/cashflows`, JSON.stringify(body), {
    headers: authCtx.hdrs,
    tags: { name: 'fp_cashflows_post' },
    timeout,
  });
  fpObserve(res, { method: 'POST', endpoint: '/api/v1/cashflows', tagName: 'fp_cashflows_post' });
  let cashflowId = parseCashflowId(res);
  if (!cashflowId && res.status === 401 && authCtx.refreshAccessToken) {
    console.warn(
      `[volume-client] 401 on cashflow — refreshing token clientTag=${clientTag} planIndex=${planIndex}`,
    );
    authCtx.ensureFresh(true);
    res = http.post(`${base}/api/v1/cashflows`, JSON.stringify(body), {
      headers: authCtx.hdrs,
      tags: { name: 'fp_cashflows_post' },
      timeout,
    });
    fpObserve(res, { method: 'POST', endpoint: '/api/v1/cashflows', tagName: 'fp_cashflows_post' });
    cashflowId = parseCashflowId(res);
  }
  return { res, cashflowId };
}
...
function advisorStaggerSeconds() {
  if (!volumeFleetStaggerEnabled()) return 0;
...
function seedAllPlansForClient({
  ...
  if (authCtx.refreshAccessToken) authCtx.ensureFresh(false);
...
export function executeFullPlatformSequence({
  ...
  if (shouldPreRunCleanup) {
    console.log(
      `[volume-cleanup] pre-run purge starting advisorSub=${advisorSub} deleteAll=${deleteAllBeforeWrite}`,
    );
