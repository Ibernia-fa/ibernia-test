...
function createClientOnce({ base, authCtx, createBody, timeout, clientTag }) {
  let lastRes = http.post(`${base}/api/v1/Clients`, JSON.stringify(createBody), {
  ...
  let parsed = parseClientCreateResponse(lastRes);
  if (parsed.id && parsed.model) {
    return { resCreate: lastRes, clientId: parsed.id, modelRaw: parsed.model };
  }
  if (lastRes.status === 401 && authCtx.refreshAccessToken) {
  ...
  if (!parsed.id) {
    console.error(
      `[volume-client] create failed clientTag=${clientTag} status=${lastRes ? lastRes.status : 'n/a'}`,
    );
  }
  return { resCreate: lastRes, clientId: parsed.id, modelRaw: parsed.model };
}
...
function clientSlotStaggerSeconds(clientIndex, advisorIndex) {
  ...
  if (clientIndex < 10) return 0;
  if (clientIndex < 14) {
    return Math.min(20, (clientIndex - 9) * 2 + (ai % 5));
  }
...
function seedAllPlansForClient({
  ...
  const existingRows = listClientPlanRows(base, hdrs, clientId, timeout);
...
function processClientSlot({
  ...
  if (existingClientId) {
    ...
    const created = createClientOnce({
      base,
      authCtx,
      createBody,
      timeout,
