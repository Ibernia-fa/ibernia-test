 ...
 * income & expense lines, contributions & withdrawals, financial record, report forecast, and wealth assets/liabilities.
 *
...
function buildPlanPhases({
  ...
  const persona = selectPersonaForClient(uniqueTag, clientIndex, planIndex);
  const planName = planTitleForPersona(persona, planIndex, planCount);
  ...
  if (sloOn) {
    ...
    completePhaseAStep(PHASE_A.ADD_SAVING_POTS, savingPotsStart, { ok: seedResult.wealthOk });
  }
...
/**
 * Full sequential journey for one user. Uses **`uniqueTag`** in client last name for **`deleteClientsAndPlansByLastNameNeedle`**.
...
export function executeFullPlatformSequence({
  ...
  if (shouldPreRunCleanup) {
    const purge = purgeK6ClientsAndPlansForAdvisor(base, hdrs, advisorSub, timeout, {
    ...
    }, { deleteAllClients: deleteAllBeforeWrite });
    console.log(
  ...
  for (let ci = 0; ci < clientCount; ci++) {
    ...
    const clientEmail = `${persona.firstName.toLowerCase()}.${persona.lastName.toLowerCase()}.${clientTag}@${domain || 'example.com'}`;
    ...
    check(resCreate, { 'fp: POST Clients accepted': () => createHttpAccepted(resCreate.status, clientId) });
    ...
    const model = applyRealisticClientProfile(modelRaw, { persona, uniqueTag: clientTag });
    model.Id = clientId;
    ...
    if (!skipDelete) {
      deleteClientsAndPlansByLastNameNeedle(base, hdrs, advisorSub, clientTag, timeout, {
