/**
 * Load test **GET /api/v1/Clients/{id}** for **`/clients/{clientId}/profile`** (open profile).
 *
 * **Concurrency:** **`constant-vus`** — per-VU seed (**POST /Clients**) then repeated **GET** under row-0 advisor token.
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/clients-profile/k6-clients-profile-get-client-load.js `
...
const SCRIPT_TAG = 'k6-clients-profile-get-client-load';
const NEEDLE_PREFIX = 'k6profgc';
const GLOBAL_SEED = '__k6ClientsProfileGetSeed';
...
const dur = new Trend('clients_profile_get_client_ms');
const errs = new Counter('clients_profile_get_client_errors');
const oks = new Counter('clients_profile_get_client_success');
const errBuckets = createHttpErrorBuckets('clients_profile_get_client_err');

export const options = {
  scenarios: {
    clients_profile_get_client: {
...
export function setup() {
  ...
  if (!ctx) {
    abortTest(SCRIPT_TAG, 'setup: login failed row 0.');
  }
  console.log(
    `[${SCRIPT_TAG}] runTag=${runTag} | VUS=${VUS} | DURATION=${DURATION} | screen=/clients/{clientId}/profile`,
  );
  return {
    base: API_BASE,
    runTag,
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'clients_profile_get_client', vu: String(__VU) },
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'clients_profile_get_client_ms',
  errMetricName: 'clients_profile_get_client_errors',
  okMetricName: 'clients_profile_get_client_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/clients-profile/reports/k6-clients-profile-get-client-load-report.json',
});
