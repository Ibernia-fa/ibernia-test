/**
 * Load test **GET /api/v1/Clients/{advisorId}/all** — client roster (context for profile / navigation).
 *
 * **Concurrency:** **`constant-vus`** — per-VU **POST /Clients** seed so the list is non-empty for that advisor’s data shape, then repeated **GET …/all**.
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/clients-profile/k6-clients-profile-get-advisor-clients-all-load.js `
...
const SCRIPT_TAG = 'k6-clients-profile-get-advisor-clients-all-load';
const NEEDLE_PREFIX = 'k6profga';
const GLOBAL_SEED = '__k6ClientsProfileAllSeed';
...
const dur = new Trend('clients_profile_get_advisor_clients_all_ms');
const errs = new Counter('clients_profile_get_advisor_clients_all_errors');
const oks = new Counter('clients_profile_get_advisor_clients_all_success');
const errBuckets = createHttpErrorBuckets('clients_profile_get_all_err');

export const options = {
  scenarios: {
    clients_profile_get_all: {
...
export default function (data) {
  ...
  const url = `${base}/api/v1/Clients/${encodeURIComponent(data.advisorSub)}/all`;
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'clients_profile_get_advisor_clients_all', vu: String(__VU) },
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  apiKey: 'get_advisor_clients_all',
  apiLabel: 'GET /api/v1/Clients/{advisorId}/all',
  method: 'GET',
  endpoint: '/api/v1/Clients/{advisorId}/all',
  expectedStatus: 200,
  durMetricName: 'clients_profile_get_advisor_clients_all_ms',
  errMetricName: 'clients_profile_get_advisor_clients_all_errors',
  okMetricName: 'clients_profile_get_advisor_clients_all_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/clients-profile/reports/k6-clients-profile-get-advisor-clients-all-load-report.json',
});
