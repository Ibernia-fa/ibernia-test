/**
 * Load test **GET /api/v1/cashflows/{cashflowId}/financial** — financial aggregate (often loaded with income UI).
 *
 * **Concurrency:** **`constant-vus`** (same seed model as **`k6-cashflows-income-get-cashflow-load.js`**).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-income/k6-cashflows-income-get-financial-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-income-get-financial-load';
const GLOBAL_SEED = '__k6CfIncomeSeed_getfin';
...
const dur = new Trend('cashflows_income_get_financial_ms');
const errs = new Counter('cashflows_income_get_financial_errors');
const oks = new Counter('cashflows_income_get_financial_success');
const errBuckets = createHttpErrorBuckets('cashflows_income_get_financial_err');

export const options = {
  scenarios: {
    cashflows_income_get_financial: {
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'cashflows_income_get_financial', vu: String(__VU) },
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_income_get_financial_ms',
  errMetricName: 'cashflows_income_get_financial_errors',
  okMetricName: 'cashflows_income_get_financial_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-income/reports/cashflows-income-get-financial-load-report.json',
});
