/**
 * Load test **GET /api/v1/cashflows/{cashflowId}** — shell data for **`/cashflows/{id}/income`**.
 *
 * **Concurrency:** **`constant-vus`** — one VU per lifecycle row (seeded cashflow per VU), or **`SHARED_CASHFLOW_ID`** + **`VUS`**. Seed runs on **first iteration per VU** (non-blocking across VUs). **`teardown()`** cleans seeded clients/plans.
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-income/k6-cashflows-income-get-cashflow-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-income-get-cashflow-load';
const GLOBAL_SEED = '__k6CfIncomeSeed_getcf';
...
const dur = new Trend('cashflows_income_get_cashflow_ms');
const errs = new Counter('cashflows_income_get_cashflow_errors');
const oks = new Counter('cashflows_income_get_cashflow_success');
const errBuckets = createHttpErrorBuckets('cashflows_income_get_cashflow_err');

export const options = {
  scenarios: {
    cashflows_income_get_cashflow: {
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'cashflows_income_get_cashflow', vu: String(__VU) },
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_income_get_cashflow_ms',
  errMetricName: 'cashflows_income_get_cashflow_errors',
  okMetricName: 'cashflows_income_get_cashflow_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-income/reports/cashflows-income-get-cashflow-load-report.json',
});
