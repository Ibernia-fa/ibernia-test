 ...
 *
 * **Seeded journey → load `GET /api/v1/Events/custom`**
 *
 * Mirrors the app flow: **login** → **POST client** → **POST cashflow (plan)** → **read-only** **`GET /api/v1/Events/default`** + **`GET …/Events/custom`** (no custom goal **POST**, so badges are not added before Wedding/Birth).
 * **Concurrent seeding:** **`setup()`** logs + **`runTag`** (no HTTP). **First iteration per VU** runs that journey in parallel; state on **`globalThis.__k6EventsCustomSeededByVu`**. Then **`constant-vus`** load **GET …/Events/custom** (**`events_custom_seeded_get`**). **`teardown()`** uses **`runTag`** + **`lib/k6-load-cleanup.js`** (events by name needle, then clients/plans by last-name needle).
 *
 * Modules: **`client_profile`**, **`cashflow`**, **`goals`**. Optional **`LIFECYCLE_MAX_USERS`**, **`RELAX_*`** (same spirit as **`k6/cashflows-timeline/k6-cashflows-timelines-financing-load.js`**).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET" k6/cashflows-timeline/k6-events-custom-seeded-load.js
 * ```
 */
...
import {
  ...
  deleteCustomEventsByNameNeedle,
...
const SCRIPT_TAG = 'k6-events-custom-seeded-load';
...
const SCENARIO_VUS = scenarioVusForPool(LIFECYCLE_USERS.length, 'k6-events-custom-seeded-load');
...
function buildCashflowBody({ clientId, clientName, advisorSub, advisorName, planName, clientBirthDateIso }) {
  return {
    ...
    planDuration: 40,
    inflationRate: 2.5,
    description: `k6 events custom seeded ${planName}`,
...
/** @param {'seed'|'teardown'} phase */
function cleanupSeededRow(base, hdrs, cashflowId, clientId, phase) {
  const p = phase === 'teardown' ? 'events_seeded_teardown' : 'events_seeded_seed';
  if (cashflowId) {
    http.del(`${base}/api/v1/cashflows/${encodeURIComponent(cashflowId)}`, null, {
      headers: hdrs,
      tags: { name: `${p}_cleanup_cf` },
      timeout: HTTP_TIMEOUT,
    });
  }
  if (clientId) {
...
function getSeedStore() {
  if (!globalThis.__k6EventsCustomSeededByVu) {
    globalThis.__k6EventsCustomSeededByVu = {};
  }
  return globalThis.__k6EventsCustomSeededByVu;
}

/**
 * First iteration per VU: login → client → cashflow → GET Events/default + GET …/Events/custom (read-only).
 * Runs concurrently across VUs.
 */
function seedVuIfNeeded(base, vuKey, idx, runTag) {
  ...
  const uniqueTag = `k6ce${runTag}vu${vuKey}`;
  const clientEmail = `k6ce.${uniqueTag}.${idx}.${Date.now()}@${domain}`;
  ...
  const resClient = http.post(`${base}/api/v1/Clients`, JSON.stringify(body), {
    headers: hdrs,
    tags: { name: 'events_seeded_seed_create_client' },
    timeout: HTTP_TIMEOUT,
  });
  ...
  const planName = `k6-ce-plan-${uniqueTag}`.slice(0, 120);
  ...
  const resEvDef = http.get(`${base}/api/v1/Events/default`, {
    headers: hdrs,
    tags: { name: 'events_seeded_seed_default' },
    timeout: HTTP_TIMEOUT,
  });
  if (relaxGoalsModule && isGoalsModuleNotActive402(resEvDef)) {
    cleanupSeededRow(base, hdrs, seededCashflowId, seededClientId, 'seed');
    abortTest(
  ...
  const verify = http.get(`${base}/api/v1/Events/custom`, {
    headers: hdrs,
    tags: { name: 'events_seeded_seed_verify_custom' },
    timeout: HTTP_TIMEOUT,
  });
  ...
  if (verify.status !== 200) {
    cleanupSeededRow(base, hdrs, seededCashflowId, seededClientId, 'seed');
    abortTest(`seed: GET Events/custom failed VU ${vuKey} (${row.email}) HTTP ${verify.status}`);
  }
  ...
  store[vuKey] = entry;
  console.log(
    `[${SCRIPT_TAG}] seed VU ${vuKey} ok: ${row.email} cashflowId=${seededCashflowId}`,
  );
  return entry;
}
...
export function teardown(data) {
  ...
  if (!runTag) return;
  const needle = `k6ce${runTag}`;
  for (let idx = 0; idx < LIFECYCLE_USERS.length; idx++) {
    const row = LIFECYCLE_USERS[idx];
    const ctx = loginUserContext(row);
    if (!ctx) continue;
    const hdrs = apiHeaders(ctx.accessToken);
    deleteCustomEventsByNameNeedle(base, hdrs, needle, HTTP_TIMEOUT, {
      list: 'events_seeded_teardown_events_custom',
      delEvent: 'events_seeded_teardown_delete_event',
    });
    deleteClientsAndPlansByLastNameNeedle(base, hdrs, ctx.advisorSub, needle, HTTP_TIMEOUT, {
      list: 'events_seeded_teardown_list_clients',
      cfList: 'events_seeded_teardown_cf_list',
      delCf: 'events_seeded_teardown_delete_cashflow',
      delClient: 'events_seeded_teardown_delete_client',
    });
  }
  delete globalThis.__k6EventsCustomSeededByVu;
}

export const options = {
  scenarios: {
    events_custom_seeded_get: {
...
export default function (data) {
  ...
  if (!e || !e.token || !e.seededCashflowId) {
    console.error(`[${SCRIPT_TAG}] VU ${vuKey} (index ${idx}): seed incomplete.`);
    return;
  }

  const hdrs = apiHeaders(e.token);
  const res = http.get(`${base}/api/v1/Events/custom`, {
    headers: hdrs,
    tags: { name: 'events_custom_seeded_get', vu: String(__VU) },
    timeout: HTTP_TIMEOUT,
  });

  check(res, {
    [`Events/custom (seeded) vu${__VU} (${e.email}): GET 200`]: (r) => r.status === 200,
  });
...
export const handleSummary = wrapHandleSummaryWithConsolidatedPerf(null, {
  moduleName: 'events-custom',
  sliceId: 'k6-events-custom-seeded-load',
});
