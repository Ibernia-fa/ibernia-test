/**
 * Load test **GET /api/v1/cashflows/{cashflowId}/financial** for **`/cashflows/{id}/reports`** (full financial record).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-reports/k6-cashflows-reports-get-financial-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-reports-get-financial-load';
const GLOBAL_SEED = '__k6CfReportsSeed_getfin';
...
const dur = new Trend('cashflows_reports_get_financial_ms');
const errs = new Counter('cashflows_reports_get_financial_errors');
const oks = new Counter('cashflows_reports_get_financial_success');
const errBuckets = createHttpErrorBuckets('cashflows_reports_get_financial_err');

export const options = {
  scenarios: {
    cashflows_reports_get_financial: {
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'cashflows_reports_get_financial', vu: String(__VU) },
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_reports_get_financial_ms',
  errMetricName: 'cashflows_reports_get_financial_errors',
  okMetricName: 'cashflows_reports_get_financial_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-reports/reports/cashflows-reports-get-financial-load-report.json',
});
