/**
 * Load test **POST /api/v1/Reports/{cashflowId}** (forecast payload) for **`/cashflows/{id}/reports`** (**200** or **204**).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-reports/k6-cashflows-reports-post-forecast-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-reports-post-forecast-load';
const GLOBAL_SEED = '__k6CfReportsSeed_postfc';
...
const dur = new Trend('cashflows_reports_post_forecast_ms');
const errs = new Counter('cashflows_reports_post_forecast_errors');
const oks = new Counter('cashflows_reports_post_forecast_success');
const errBuckets = createHttpErrorBuckets('cashflows_reports_post_forecast_err');

export const options = {
  scenarios: {
    cashflows_reports_post_forecast: {
...
export default function (data) {
  ...
  const inflationRate = 2.25 + ((__VU * 3 + __ITER) % 12) / 10;
  const url = `${base}/api/v1/Reports/${encodeURIComponent(e.seededCashflowId)}?inflationRate=${inflationRate}`;
  const body = buildReportForecastPayload(__VU, __ITER, runTag);
  const res = http.post(url, JSON.stringify(body), {
    headers: hdrs,
    tags: { name: 'cashflows_reports_post_forecast', vu: String(__VU) },
  ...
  recordOutcomeWithBuckets(
    ...
    res,
    200,
    `${SCRIPT_TAG} POST Reports forecast vu${__VU}`,
    errBuckets.buckets,
    REPORTS_HTTP_SUCCESS,
  );
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  apiKey: 'post_reports_forecast',
  apiLabel: 'POST /api/v1/Reports/{cashflowId} (forecast)',
  method: 'POST',
  endpoint: '/api/v1/Reports/{cashflowId}',
  expectedStatus: 200,
  expectedStatusDisplay: '200 or 204',
  durMetricName: 'cashflows_reports_post_forecast_ms',
  errMetricName: 'cashflows_reports_post_forecast_errors',
  okMetricName: 'cashflows_reports_post_forecast_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-reports/reports/cashflows-reports-post-forecast-load-report.json',
});
