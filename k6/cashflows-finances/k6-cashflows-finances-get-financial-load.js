/**
 * Load test **GET /api/v1/cashflows/{cashflowId}/financial** — full financial record for **`/cashflows/{id}/finances`**.
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-finances/k6-cashflows-finances-get-financial-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-finances-get-financial-load';
const GLOBAL_SEED = '__k6CfFinancesSeed_getfin';
...
const dur = new Trend('cashflows_finances_get_financial_ms');
const errs = new Counter('cashflows_finances_get_financial_errors');
const oks = new Counter('cashflows_finances_get_financial_success');
const errBuckets = createHttpErrorBuckets('cashflows_finances_get_financial_err');

export const options = {
  ...teardownTimeoutOption(LIFECYCLE_USERS.length),
  scenarios: {
    cashflows_finances_get_financial: {
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'cashflows_finances_get_financial', vu: String(__VU) },
  ...
  recordOutcomeWithBuckets(
    ...
    res,
    200,
    `${SCRIPT_TAG} GET financial vu${__VU}`,
    errBuckets.buckets,
  );
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_finances_get_financial_ms',
  errMetricName: 'cashflows_finances_get_financial_errors',
  okMetricName: 'cashflows_finances_get_financial_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-finances/reports/cashflows-finances-get-financial-load-report.json',
});
