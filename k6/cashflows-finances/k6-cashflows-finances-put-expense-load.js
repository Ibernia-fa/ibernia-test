/**
 * Load test **PUT /api/v1/cashflows/{cashflowId}/financial/expense** for **`/cashflows/{id}/finances`** (POST seed → PUT → DELETE cleanup).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-finances/k6-cashflows-finances-put-expense-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-finances-put-expense-load';
const GLOBAL_SEED = '__k6CfFinancesSeed_putexp';
...
const dur = new Trend('cashflows_finances_put_expense_ms');
const errs = new Counter('cashflows_finances_put_expense_errors');
const oks = new Counter('cashflows_finances_put_expense_success');
const errBuckets = createHttpErrorBuckets('cashflows_finances_put_expense_err', { lineResolve: true });

export const options = {
  scenarios: {
    cashflows_finances_put_expense: {
...
export default function (data) {
  ...
  const { stablePrefix, descFull } = buildLoadTestLineNeedles('Finances expense load test update', runTag, __VU, __ITER);
  ...
  const postRes = http.post(postUrl, JSON.stringify(createBody), {
    headers: hdrs,
    tags: { name: 'cashflows_finances_put_exp_seed', vu: String(__VU) },
  ...
  const resolved = resolveExpenseLineAfterPost({
    ...
    stablePrefix,
    vuTag: String(__VU),
    tagName: 'cashflows_finances_put_exp_getid',
  });
  if (!resolved) {
    errs.add(1);
  ...
  const putBody = buildMinimalExpenseLineItem({
    id: resolved.id,
    description: resolved.description,
    amount: 555 + (__ITER % 15),
  });
  const putRes = http.put(postUrl, JSON.stringify(putBody), {
    headers: hdrs,
    tags: { name: 'cashflows_finances_put_expense', vu: String(__VU) },
    timeout: HTTP_TIMEOUT,
  });
  const putOk = recordOutcomeWithBuckets(
    ...
    putRes,
    200,
    `${SCRIPT_TAG} PUT financial/expense vu${__VU}`,
    errBuckets.buckets,
  );
  if (putOk) {
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  apiKey: 'put_financial_expense',
  apiLabel: 'PUT /api/v1/cashflows/{cashflowId}/financial/expense',
  method: 'PUT',
  endpoint: '/api/v1/cashflows/{cashflowId}/financial/expense',
  expectedStatus: 200,
  durMetricName: 'cashflows_finances_put_expense_ms',
  errMetricName: 'cashflows_finances_put_expense_errors',
  okMetricName: 'cashflows_finances_put_expense_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-finances/reports/cashflows-finances-put-expense-load-report.json',
});
