/**
 * Load test **POST /api/v1/cashflows/{cashflowId}/financial/income** for **`/cashflows/{id}/finances`** (then DELETE cleanup).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-finances/k6-cashflows-finances-post-income-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-finances-post-income-load';
const GLOBAL_SEED = '__k6CfFinancesSeed_postinc';
...
const dur = new Trend('cashflows_finances_post_income_ms');
const errs = new Counter('cashflows_finances_post_income_errors');
const oks = new Counter('cashflows_finances_post_income_success');
const errBuckets = createHttpErrorBuckets('cashflows_finances_post_income_err');

export const options = {
  scenarios: {
    cashflows_finances_post_income: {
...
function deleteIncomeLine(base, hdrs, cashflowId, postRes, needle, stablePrefix) {
  const resolved = resolveIncomeLineAfterPost({
    ...
    stablePrefix,
    vuTag: String(__VU),
    tagName: 'cashflows_finances_post_inc_cleanup_get',
  });
  if (!resolved) {
    console.error(`[${SCRIPT_TAG}] cleanup: could not resolve line vu${__VU}`);
    return;
  }
  ...
  const delRes = http.del(
    ...
    {
      headers: hdrs,
      tags: { name: 'cashflows_finances_post_inc_cleanup_del', vu: String(__VU) },
...
export default function (data) {
  ...
  const { stablePrefix, descFull } = buildLoadTestLineNeedles('Finances income load test post', runTag, __VU, __ITER);
  ...
  const res = http.post(url, JSON.stringify(body), {
    headers: hdrs,
    tags: { name: 'cashflows_finances_post_income', vu: String(__VU) },
    timeout: HTTP_TIMEOUT,
  });
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_finances_post_income_ms',
  errMetricName: 'cashflows_finances_post_income_errors',
  okMetricName: 'cashflows_finances_post_income_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-finances/reports/cashflows-finances-post-income-load-report.json',
});
