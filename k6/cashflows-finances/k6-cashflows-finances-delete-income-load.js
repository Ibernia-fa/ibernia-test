/**
 * Load test **DELETE /api/v1/cashflows/{cashflowId}/financial/income** for **`/cashflows/{id}/finances`** (POST seed → DELETE).
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/cashflows-finances/k6-cashflows-finances-delete-income-load.js -e SIGNUP_ROPC_CLIENT_ID=k6-load-test-client -e SIGNUP_ROPC_CLIENT_SECRET="YOUR_SECRET"
 * ```
 */
...
const SCRIPT_TAG = 'k6-cashflows-finances-delete-income-load';
const GLOBAL_SEED = '__k6CfFinancesSeed_delinc';
...
const dur = new Trend('cashflows_finances_delete_income_ms');
const errs = new Counter('cashflows_finances_delete_income_errors');
const oks = new Counter('cashflows_finances_delete_income_success');
const errBuckets = createHttpErrorBuckets('cashflows_finances_delete_income_err', { lineResolve: true });

export const options = {
  scenarios: {
    cashflows_finances_delete_income: {
...
export default function (data) {
  ...
  const { stablePrefix, descFull } = buildLoadTestLineNeedles('Finances income load test delete', runTag, __VU, __ITER);
  ...
  const postRes = http.post(postUrl, JSON.stringify(createBody), {
    headers: hdrs,
    tags: { name: 'cashflows_finances_del_inc_seed', vu: String(__VU) },
  ...
  const resolved = resolveIncomeLineAfterPost({
    ...
    stablePrefix,
    vuTag: String(__VU),
    tagName: 'cashflows_finances_del_inc_getid',
  });
  if (!resolved) {
    errs.add(1);
  ...
  const delRes = http.del(delUrl, JSON.stringify(delBody), {
    headers: hdrs,
    tags: { name: 'cashflows_finances_delete_income', vu: String(__VU) },
    timeout: HTTP_TIMEOUT,
  });
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  expectedStatus: 200,
  durMetricName: 'cashflows_finances_delete_income_ms',
  errMetricName: 'cashflows_finances_delete_income_errors',
  okMetricName: 'cashflows_finances_delete_income_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/cashflows-finances/reports/cashflows-finances-delete-income-load-report.json',
});
