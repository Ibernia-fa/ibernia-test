/**
 * Load test **GET /api/v1/Clients/{advisorId}/all** for **`https://dev.ibernia.it/clients`** (advisor client list).
 *
 * **Concurrency:** **`constant-vus`** — first iteration per VU (concurrent) seeds **two** clients; load phase repeats **GET …/all**.
 *
 * ```powershell
...
const SCRIPT_TAG = 'k6-clients-get-advisor-all-load';
...
const dur = new Trend('clients_get_all_ms');
const errs = new Counter('clients_get_all_errors');
const oks = new Counter('clients_get_all_success');
const errBuckets = createHttpErrorBuckets('clients_get_all_err');

export const options = {
  scenarios: {
    clients_get_advisor_all: {
...
function getSeedStore() {
  if (!globalThis.__k6ClientsGetAllSeedByVu) {
    globalThis.__k6ClientsGetAllSeedByVu = {};
  }
  return globalThis.__k6ClientsGetAllSeedByVu;
}
...
function seedVuIfNeeded(base, vuKey, auth) {
  ...
  for (let i = 0; i < 2; i++) {
    const tag = `k6cgetall${runTag}-vu${vuKey}-${i}`;
    const clientEmail = `k6cgetall.${tag}.${i}@${domain}`;
    ...
    const res = http.post(`${base}/api/v1/Clients`, JSON.stringify(body), {
      headers: hdrs,
      tags: { name: 'clients_get_all_seed_create', vu: String(vuKey) },
    ...
    if (!createHttpAccepted(res.status, id) || !id) {
      for (let j = 0; j < ids.length; j++) {
        http.del(`${base}/api/v1/Clients/${encodeURIComponent(ids[j])}`, null, {
          headers: hdrs,
          tags: { name: 'clients_get_all_seed_cleanup', vu: String(vuKey) },
          timeout: HTTP_TIMEOUT,
        });
      }
  ...
  const verify = http.get(`${base}/api/v1/Clients/${encodeURIComponent(advisorSub)}/all`, {
    headers: hdrs,
    tags: { name: 'clients_get_all_seed_verify', vu: String(vuKey) },
    timeout: HTTP_TIMEOUT,
  });
  if (!(verify.status === 200 && listResponseContainsBothIds(verify, ids[0], ids[1]))) {
    for (let j = 0; j < ids.length; j++) {
      http.del(`${base}/api/v1/Clients/${encodeURIComponent(ids[j])}`, null, {
        headers: hdrs,
        tags: { name: 'clients_get_all_seed_cleanup', vu: String(vuKey) },
    ...
    abortTest(SCRIPT_TAG, `seed: verify GET /all failed VU ${vuKey} HTTP ${verify.status}`);
  }

  const entry = { token, advisorSub, ids };
  store[vuKey] = entry;
  console.log(`[${SCRIPT_TAG}] seed VU ${vuKey} ok ids=${ids.join(',')}`);
  return entry;
}
...
export function teardown(data) {
  ...
  deleteClientsAndPlansByLastNameNeedle(
    ...
    data.advisorSub,
    `k6cgetall${data.runTag}`,
    HTTP_TIMEOUT,
    {
      list: 'clients_get_all_teardown_list',
      cfList: 'clients_get_all_teardown_cf_list',
      delCf: 'clients_get_all_teardown_del_cf',
      delClient: 'clients_get_all_teardown_del_client',
    },
  );
  try {
    delete globalThis.__k6ClientsGetAllSeedByVu;
  } catch {
    /* ignore */
  }
}
...
export default function (data) {
  ...
  const res = http.get(url, {
    headers: hdrs,
    tags: { name: 'clients_get_advisor_all', vu: String(__VU) },
    timeout: HTTP_TIMEOUT,
  });
  recordOutcomeWithBuckets(
    dur,
    ...
    res,
    200,
    `${SCRIPT_TAG} GET /Clients/{advisorId}/all vu${__VU}`,
    errBuckets.buckets,
  );
  check(res, {
    [`${SCRIPT_TAG} body has seeded ids`]: (r) =>
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  apiKey: 'get_clients_all',
  apiLabel: 'GET /api/v1/Clients/{advisorId}/all',
  method: 'GET',
  endpoint: '/api/v1/Clients/{advisorId}/all',
  expectedStatus: 200,
  durMetricName: 'clients_get_all_ms',
  errMetricName: 'clients_get_all_errors',
  okMetricName: 'clients_get_all_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/clients/reports/k6-clients-get-advisor-all-load-report.json',
});
