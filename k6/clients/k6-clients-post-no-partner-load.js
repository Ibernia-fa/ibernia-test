/**
 * Load test **POST /api/v1/Clients** (create **without** partner) for **`https://dev.ibernia.it/clients`**.
 *
 * **Concurrency:** **`constant-vus`** — each VU/iteration **POST**s a unique client, then **DELETE**s it so the DB does not grow.
 *
 * ```powershell
 * cd C:\Users\gulle\source\repos\load-testing-k6
 * k6 run k6/clients/k6-clients-post-no-partner-load.js `
...
const SCRIPT_TAG = 'k6-clients-post-no-partner-load';
...
const dur = new Trend('clients_post_np_ms');
const errs = new Counter('clients_post_np_errors');
const oks = new Counter('clients_post_np_success');
const errBuckets = createHttpErrorBuckets('clients_post_np_err');

export const options = {
  scenarios: {
    clients_post_no_partner: {
...
export function setup() {
  ...
  console.log(
    `[${SCRIPT_TAG}] runTag=${runTag} | VUS=${VUS} | DURATION=${DURATION} | POST /api/v1/Clients (no partner)`,
  );
  return {
    base: API_BASE,
    runTag,
...
export function teardown(data) {
  ...
  deleteClientsAndPlansByLastNameNeedle(
    ...
    data.advisorSub,
    `k6cpostnp${data.runTag}`,
    HTTP_TIMEOUT,
    {
      list: 'clients_post_np_teardown_list',
      cfList: 'clients_post_np_teardown_cf_list',
      delCf: 'clients_post_np_teardown_del_cf',
      delClient: 'clients_post_np_teardown_del_client',
    },
  );
}
...
export default function (data) {
  ...
  const uniqueTag = `k6cpostnp${data.runTag}vu${__VU}it${__ITER}`;
  const clientEmail = `k6cpostnp.${uniqueTag}.${Date.now()}@${domain}`;
  ...
  const res = http.post(`${base}/api/v1/Clients`, JSON.stringify(body), {
    headers: hdrs,
    tags: { name: 'clients_post_np', vu: String(__VU) },
  ...
  recordOutcomeWithBuckets(
    ...
    res,
    201,
    `${SCRIPT_TAG} POST /Clients (no partner) vu${__VU}`,
    errBuckets.buckets,
    [200, 201],
  );
  ...
  if (id) {
    http.del(`${base}/api/v1/Clients/${encodeURIComponent(id)}`, null, {
      headers: hdrs,
      tags: { name: 'clients_post_np_iter_cleanup', vu: String(__VU) },
      timeout: HTTP_TIMEOUT,
    });
  }
...
export const handleSummary = singleApiHandleSummaryFactory({
  ...
  apiKey: 'post_clients_no_partner',
  apiLabel: 'POST /api/v1/Clients (no partner)',
  method: 'POST',
  endpoint: '/api/v1/Clients',
  expectedStatus: 201,
  expectedStatusDisplay: '200 or 201',
  durMetricName: 'clients_post_np_ms',
  errMetricName: 'clients_post_np_errors',
  okMetricName: 'clients_post_np_success',
  errBucketDefs: errBuckets.defs,
  reportFilename: 'k6/clients/reports/k6-clients-post-no-partner-load-report.json',
});
