...
export class AddWithdrawalComponent {
  ...
  get dialogTitle(): string {
    ...
    const description = (this.withdrawalForm?.get('description')?.value ?? '').toString().trim();
    return description || 'Edit Withdrawal';
  }
  ...
  private applyDefaultDescriptionFromSavingPot(savingPotId: string | null): void {
    ...
    const shouldReplace =
      ...
      (!this.lastAutoDescription && this.isAutoGeneratedDescription(currentValue));
  ...
  private isAutoGeneratedDescription(description: string): boolean {
    const normalized = (description ?? '').toString().trim();
    return (
      normalized === 'Withdrawal from' ||
      normalized.startsWith('Withdrawal from ')
    );
  }
