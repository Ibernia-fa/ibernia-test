...
export class AddEmergenciesComponent {
  ...
  constructor(
    ...
    this.isDefaultEmergency = hasExplicitDefaultFlag
      ...
      : !!data.emergency &&
        this.DEFAULT_EMERGENCY_NAMES.has(data.emergency.name);
    ...
    this.form = this.fb.group({
      ...
      insuranceAmount: [
        this.insuranceCostTemplate?.amount === 0
          ? null
          : (this.insuranceCostTemplate?.amount ?? null),
        [Validators.min(0)],
      ],
  ...
  private patchForm(e: Emergency): void {
    this.form.patchValue({
      ...
      insuranceAmount:
        e.insuranceCost?.amount === 0 ? null : e.insuranceCost?.amount,
      insuranceCycleId: e.insuranceCost?.cycle?.id ?? 'annual',
      coverage: e.coverage === 0 ? null : e.coverage,
  ...
  onSave(): void {
    ...
    const policyStatus = isWill
      ? this.NOT_COVERED_STATUS_ID
      : form.policyStatus;
    const insuranceAmount =
      isWill || isUncovered ? 0 : (form.insuranceAmount ?? 0);
    const insuranceCycleId =
      isWill || isUncovered
        ? (this.insuranceCycles[0]?.id ?? form.insuranceCycleId ?? null)
        : (form.insuranceCycleId ?? this.insuranceCycles[0]?.id ?? null);
    const coverageAdequacy =
      isWill || isUncovered
    ...
    const insuranceCost: Money | null = isWill
      ? null
      : insuranceAmount != null &&
        ...
        ? {
            ...
            cycle: insuranceCycleId
              ? { id: insuranceCycleId, description: null }
              : null,
          }
        : null;
  ...
  private updateValidatorsForPolicyStatus(status: number | null): void {
    ...
    if (
      !insuranceAmountCtrl ||
      !insuranceCycleIdCtrl ||
      !coverageAdequacyCtrl ||
      !coverageCtrl
    ) {
      return;
    }
  ...
  private updateValidatorsForFormByType(): void {
    if (this.isWill) {
      ...
      this.updateValidatorsForPolicyStatus(
        this.form.get('policyStatus')?.value ?? null,
      );
  ...
  get isWill(): boolean {
    ...
    return (
      this.data.emergencyTypes?.length === 1 &&
      this.data.emergencyTypes[0].id === 2
    );
  }
